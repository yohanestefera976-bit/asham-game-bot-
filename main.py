...
ADMIN_ID = 7626795163  # ዮሐንስ ትክክለኛ አድሚን ID
...
def register_user(user_id, username, full_name, phone_number):
    conn = sqlite3.connect("asham_games.db")
    cursor = conn.cursor()
    ...
    if not user:
        cursor.execute(
            "INSERT INTO users (user_id, username, full_name, phone_number, balance) VALUES (?, ?, ?, ?, ?)",
            (user_id, username, full_name, phone_number, 10.0)
        )
        conn.commit()
        conn.close()
        return 10.0
...
def get_main_menu():
    ...
    builder.button(text="🛠 ቴሌግራም ሳፖርት (Support)")
...
async def start_handler(message: types.Message, state: FSMContext):
    ...
    phone_kb = ReplyKeyboardMarkup(
        keyboard=[
            [KeyboardButton(text="📱 ስልክ ቁጥሬን አጋራ", request_contact=True)]
        ],
    ...
    await message.answer(
        "ሰላም ዮሐንስ! ወደ **Asham Games** በደህና መጡ። 🎮\n\n"
        "ለመመዝገብ እና ቦቱን ሙሉ በሙሉ ለመጠቀም እባክዎ ከታች ያለውን አዝራር በመጫን **ስልክ ቁጥርዎን** ያጋሩን:",
        parse_mode="Markdown",
...
async def contact_handler(message: types.Message):
    ...
    bonus_given = register_user(user.id, user.username or "", user.full_name, phone_number)
...
@dp.message(F.text == "🛠 ቴሌግራም ሳፖርት (Support)")
async def support_handler(message: types.Message):
    support_kb = InlineKeyboardMarkup(
        inline_keyboard=[
            [InlineKeyboardButton(text="💬 በቀጥታ አድሚን ማነጋገር", url="https://t.me/yohannes")]  # ዩዘርኔምዎን እዚህ መቀየር ይችላሉ
        ]
    )
    await message.answer(
        "🛠 **የደንበኞች ማዕከል (Support)**\n\n"
        "ከገንዘብ ግብይት ወይም ከጨዋታዎች ጋር የተያያዘ ማንኛውም ጥያቄ ወይም ችግር ካለዎት ከታች ባለው አዝራር በመጫን በቀጥታ ማነጋገር ይችላሉ!",
        parse_mode="Markdown",
