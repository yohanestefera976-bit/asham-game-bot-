...
async def deposit_handler(message: types.Message, state: FSMContext):
    ...
    await message.answer(
        "💳 **ገንዘብ ገቢ ማድረግ**\n\n"
        "ወደ አካውንትዎ ማስገባት የሚፈልጉትን የብር መጠን ይጻፉ (ለምሳሌ: 1000 ለሙከራ):",
        parse_mode="Markdown",
...
async def process_deposit_amount(message: types.Message, state: FSMContext):
    if not message.text.isdigit():
        await message.answer("⚠️ እባክዎ ትክክለኛ ቁጥር ብቻ ይጻፉ (ለምሳሌ: 1000):")
        return
    ...
    deposit_info = (
        ...
        "ገንዘቡን ካስተላለፉ በኋላ የትራንዛክሽን ቁጥሩን (Transaction ID / UTR) ወይም ደረሰኝ ቁጥር በዚህ ቦት ላይ ይጻፉልን:"
    )
...
async def process_deposit_tx(message: types.Message, state: FSMContext):
    ...
    # 1. ሒሳቡን በቀጥታ ለተጠቃሚው መጨመር (Automatic Top-up for Testing)
