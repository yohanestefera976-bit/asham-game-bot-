...
BASE_WEB_APP_URL = "https://dash5.bet/en/casino?game=%2Fkeno5526&returnUrl=casino"
...
def get_user_balance_and_phone(user_id: int):
    conn = sqlite3.connect("users.db")
    cursor = conn.cursor()
...
async def cmd_start(message: types.Message):
    ...
    user_data = get_user_balance_and_phone(user_id)
    if user_data and user_data[1]:
        ...
        await message.answer(
            ...
            f"ለመመዝገብ እና ሽልማቱን ሙሉ በሙሉ ለመጠቀም እባክዎ ከታች ያለውን አዝራር በመጫን ስልክ ቁጥርዎን ያጋሩ:",
            parse_mode="HTML",
...
async def menu_balance(message: types.Message):
    user_data = get_user_balance_and_phone(message.from_user.id)
...
async def menu_games(message: types.Message):
    ...
    # የቴሌግራም ID እና ስልክ ቁጥሩን ከሊንክ ጋር አብሮ ማያያዝ
    user_specific_url = f"{BASE_WEB_APP_URL}&telegram_id={user_id}&phone={phone_number}"
    
    keyboard = InlineKeyboardMarkup(
        inline_keyboard=[
            [InlineKeyboardButton(text="🚀 ኬኖ ጨዋታውን ክፈት", web_app=WebAppInfo(url=user_specific_url))]
        ]
    )
