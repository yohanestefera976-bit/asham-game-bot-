...
# እባክዎ ትክክለኛውን የቦት ቶከንዎ እዚህ ያስገቡ
TOKEN = "YOUR_BOT_TOKEN_HERE"
ADMIN_ID = 7131094446  # የዮሐንስ አድሚን ID
WEB_APP_URL = "https://dash5.bet/"
...
async def cmd_start(message: types.Message):
    ...
    # ተጠቃሚው ስልክ ቁጥር አስቀድሞ ሰጥቷል ወይ እንይ
    user_data = get_user_balance(user_id)
    if user_data and user_data[1]:
        # ስልክ ቁጥር ካለው ዋናውን ሜኑ አሳይ
...
async def menu_games(message: types.Message):
    keyboard = InlineKeyboardMarkup(
        inline_keyboard=[
            [InlineKeyboardButton(text="🚀 ጨዋታውን በ Mini App ክፈት", web_app=WebAppInfo(url=WEB_APP_URL))]
        ]
    )
    await message.answer("🎮 ከታች ያለውን በመጫን ጨዋታዎችን መጫወት ይችላሉ፦", reply_markup=keyboard)
...
async def web_server():
    ...
    await runner.setup()
    site = web.AppNet(runner, "0.0.0.0", 10000)
    await site.start()
